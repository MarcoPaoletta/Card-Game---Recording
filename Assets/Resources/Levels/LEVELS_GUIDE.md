...
## Solvencia: patrón impasable a evitar (2026-05-19 update)

**Mecánica del juego**: al clickear un chunk, intenta volar en su `dir`. Si su path tiene una celda de otro chunk, ese chunk es **blocker** → el chunk clickeado no sale (bumpea contra el blocker). Hay que clickear primero al blocker para liberar el path. Si dos chunks **se bloquean mutuamente** (ciclo de longitud 2) → nivel **impasable** (no hay orden de clicks que destrabe).

**Patrón impasable más común** (lo que pasaba con el zorro pre-fix):
- Dos chunks en la **misma fila**, A a la izquierda de B, A `dir=Right` y B `dir=Left` → cada uno bloquea al otro.
- Análogo en columna: A arriba de B, A `dir=Down` y B `dir=Up`.

**Fix automático en `_gridded.ps1`: nearest-exit**. Cada chunk apunta al borde más cercano del bbox del dibujo:
- Si `avgX < bboxCenterX` → `dir=Left`. Si `>=` → `dir=Right`. (Análogo Up/Down para chunks verticales.)
- Resultado: dos chunks en misma fila o (a) ambos del mismo lado → mismo dir (solvable clickeando downstream-first), o (b) uno en cada lado → apuntan AWAY (sin bloqueo mutuo). Nunca se enfrentan.

El script tiene un **detector de face-off pairs post-emisión** que warnea si quedó algún par enfrentado: `WARNING: N pares face-to-face (impasable, revisar a mano)`. Si aparece, hay que ajustar manualmente la dir de uno de los dos.
...
**Regla de paridad actualizada (2026-05-19 tarde): SÍ auto-fix con prioridad de mínima disrupción.** El pipeline ahora arregla automaticamente la paridad impar con dos modos:
- **Singletons (count == 1)**: AGREGAR un vecino bg adyacente (extiende el feature a 2-cell). Preserva features minimos como ojos sueltos o la lengua del perro.
- **Count >= 3 impar**: REMOVER la celda con más vecinos bg (la mas exterior). Tie-break por max distancia al centroide del color. Asi se quita una "punta" del borde casi sin afectar la silueta.

El script reporta cada operacion: `-({x},{y}) {Color} [bg-vecinos=N]` para removes, `+({x},{y}) {Color} (singleton extendido)` para adds. Si no hay vecinos bg disponibles para un singleton enclavado, cae a remove.
...
- **2026-05-19**: **`BeltPreset.anchorMode` — modo de anclaje por preset** ([Assets/Scripts/Data/BeltAnchorMode.cs](Assets/Scripts/Data/BeltAnchorMode.cs)). Cada preset declara cómo `BeltRepositioner` calcula su "borde interior" (el que pega al gap con las orders): **`CenterColumn`** (default) filtra renderers al rango X de las orders — bueno para ∩-arcos donde el tubo central (apex) está arriba de las orders. **`NearestPortal`** usa el control point del path más cercano a las orders en Z — bueno para belts rectos o asimétricos. **`FullBounds`** = bbox completo (comportamiento más viejo, belts compactos). El `BeltOuterEdgeAnchor` (encuadre de cámara) siempre usa bbox completo de renderers, sin importar el modo. Default es `CenterColumn`, asi presets viejos no rompen.
...
- **2026-05-19 (tarde, paridad + solvencia + singletons)**: **Tres fixes coordinados en `_gridded.ps1`** después de generar niveles 9-13:
  1. **Auto-fix de paridad** (antes: solo reportaba). Para colores impares: si count==1 (singleton), AGREGAR vecino bg para preservar el feature (caso testigo: lengua del perro como singleton Rojo (8,15), antes se removía perdiendo la feature → ahora se extiende a (8,15)+(8,16)). Para count>=3, REMOVER la celda con más vecinos bg (más exterior, mínima disrupción visual). Output: `Paridad auto-fix (N ops): -(x,y) Color [bg-vecinos=N]; +(x,y) Color (singleton extendido)`.
  2. **Solvencia por nearest-exit**. El alternador ciego Left/Right + el dir del singleton-steal creaban pares face-to-face en misma fila/col (zorro pre-fix tenía esto). Nueva regla: cada chunk apunta al borde más cercano del bbox (`avgX < center → Left`, sino `Right`; idem Y para verticales). Aplica también a stolen chunks (antes los skipeaba → creaba pares). El JSON re-ordena cells por dir al emitir, asi sobrescribir dir no rompe la chain.
  3. **Detector de face-off integrado**. Post-emisión, el script verifica para cada par de chunks (A, B) en misma fila/col si A→B y B→A en `dir`. Si encuentra, `WARNING: N pares face-to-face`. Las imágenes 5-13 dieron 0 warnings tras el fix.
  Lección: solvencia es algorítmica, no estética — verificar siempre con un check post-emisión. Y "auto-fix" tiene que respetar features mínimas (singletons = extender, no borrar).
- **2026-05-19**: **Bug crítico del pipeline: filtraba TODAS las celdas color-fondo, perdiendo blancos internos a la silueta**. Caso testigo: Sonic perdía guantes/zapatos/ojos blancos (cells DENTRO del outline azul); Zorro perdía panza/pecho blancos; Perro perdía ojos blancos. La regla antigua era "si el sample del cell ≈ bg color, skipear" — pero un blanco rodeado de outline negro NO es fondo, es parte del dibujo. **Fix en `_gridded.ps1`**: tras clasificar `$isBg`, hacer **flood-fill desde los bordes del grid solo a través de celdas bg** → marca `$outsideBg`. Solo skipear cells donde `isBg && outsideBg`. Cells bg-color NO alcanzadas por el flood (encerradas por outline) se emiten como celdas normales con el color bg (queda como entry "Blanco" en la paleta del nivel). Output del script ahora reporta `Enclosed bg cells (interior, no skipeadas): N`. Resultado en niveles 9–13: Sonic +46 cells, Zorro +122, Perro +4, Unicornio +32 al regenerar. Lección general: cualquier filtro por color tiene que pensar **topología** (dentro vs afuera), no solo similaridad.
